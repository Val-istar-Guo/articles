...
2. `Kubelet` -> `cri-containerd` -> `containerd` -> `containerd-shim` -> `runc`
3. `Kubelet` -> `docker-shim` -> `docker` -> `containerd` -> `containerd-shim` -> `runc`
...
`container`也可以进入[github containerd/containerd](https://github.com/containerd/containerd/tree/main)下载并安装。
